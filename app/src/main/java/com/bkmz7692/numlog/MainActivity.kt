        ...
        fun calculateNumerologicalMatrix(day: EditText, month: EditText, years: EditText ) {
            ...
            if (day.text.toString().toInt() in 1..31 && month.text.toString().toInt()<=12 && years.text.toString().toInt()>=1900 ){
                if (!(day.text.toString().toInt()>28 && month.text.toString().toInt() == 2)) {
                    ...
                    if (one == two) {
                        psih.text = "III"
                        psih_ras.text = "Третий психотип- Психотип “сам себе начальник, сам себе подчиненный” - сколько придумал, столько может реализовать идей, мыслей, замыслов. \n" +
                                "Потенциал  целостный, уравновешенный. Вы увлечены собой и своими задачами, легко переносите одиночество, живёте “параллельно” коллективу. У вас высокая степень самодостаточности и потребность в свободе.\n" +
                                "Из-за своей свободолюбивости, не очень любите подчинятся, или управлять большой командой. Выполняете  эту функцию пока, это выгодно."
                    }
                    //Вычисление возраста:
                    ...
                    val c_year = LineDataSet(c_year_chart, "Текущий год")
